# StudentCareer+

**Earn Early. Build Your Career.**

StudentCareer+ is a career development platform and two-sided marketplace designed for business-focused students to gain real-world experience, build professional credibility, and earn money while studying.

## 🚀 Project Overview

**StudentCareer+** bridges the gap between academic theory and real-world execution.  
Unlike traditional career platforms focused on graduates, StudentCareer+ empowers students *before graduation* by providing access to real opportunities, peer knowledge, verified success stories, and professional networking.

## 🎯 Problem

Most students face the same challenges:
- Strong theoretical knowledge but little practical experience  
- No clear path to earning during studies  
- Lack of transparent, real examples from peers  
- Fragmented tools

Existing platforms focus on corporate hiring and graduates, not early-stage student execution.

## 💡 Solution

StudentCareer+ creates a **student-only ecosystem** that combines:
- Knowledge sharing through student-written blogs  
- Verified success stories with earnings transparency  
- Internships, competitions, and sponsored opportunities  
- Student networking and collaboration  
- Career-focused student profiles  

All in one focused, student-centric product.

## 👥 Target Audience

### Primary Users
- University students (18–25)
- Majors:  
  - Business Administration  
  - Marketing  
  - Economics  
  - Entrepreneurship  
  - Management  

### Secondary Users
- Companies hiring interns
- Competition and program organizers
- Educational platforms 

### Geographic Focus
🌍 Global (students worldwide)

## 🧩 Core Features

### 1. Blog
Students share actionable content about:
- Career growth
- Freelancing and startups
- Tools, platforms, and workflows
- How to earn money during studies

**Key functionality:**
- Write and publish articles
- Add tags for discoverability
- Subscribe to authors
- Receive alerts for new content

### 2. Stories
A dedicated space for verified student success stories.

Each story includes:
- Description of the journey
- Earnings (if applicable)
- Tools and platforms used
- Timeframe and expenses

**Goal:** Inspire through real, transparent examples.

### 3. Opportunities
Centralized hub for:
- Internships
- Business competitions
- Sponsored programs and courses

Students can:
- Browse opportunities
- Save them
- Track deadlines

All listings are published by verified companies via paid plans.

### 4. Networking
- Send and accept connection requests
- Private messaging after connection
- Collaboration and idea exchange

A safe, student-focused networking environment.

### 5. Profile & Alerts
Each student profile includes:
- Photo and bio
- Desired role or career goal
- Followers and connections
- Published articles and stories
- LinkedIn integration

Alerts notify users about:
- New connection requests
- Messages
- New articles from subscribed blogs

## 💼 Business Model

StudentCareer+ operates as a **two-sided marketplace (B2C + B2B)**.

### B2B — *StudentCareer for Business*

Designed for companies and organizers seeking student talent and exposure.

**Plans:**
- **Internship Plan** — `$69/month`  
  - Up to 3 active internship listings  
  - Application analytics  

- **Competition Spotlight** — `$349/post`  
  - 365-day visibility  
  - Featured placement  

- **Sponsored Brand** — `$89/month`  
  - Verified company profile  
  - Custom company page design  
  - Priority listing placement  

### B2C — *StudentCareer+ Premium*

Freemium model for students.

**StudentPlus — `$4.99/month`:**
- Profile analytics (views by companies)
- Advanced profile customization
- Premium member badge
- AI-powered Career Advisor

## 🧠 Product Vision

**Vision**  
To become the global standard platform where students build careers and income before graduating.

**Mission**  
Empower students to gain experience, earn money, and grow professionally while studying.

## 🏷 Brand Strategy

**Brand Personality:**  
Ambitious, practical, supportive, modern

**Tone of Voice:**  
Friendly, professional, motivating

**Values:**
- Action over theory  
- Transparency  
- Growth mindset  
- Community  
- Equal opportunity  

## 🎨 Visual Identity

- **Primary Color:** Deep Purple — ambition, creativity, professional growth  
- **Secondary Color:** White — clarity, trust, simplicity  

Minimalist, modern UI focused on readability and usability.

## 📊 Market Opportunity

- 200M+ university students globally  
- Rapid growth of remote work and freelancing  
- Rising demand for early career monetization  
- Companies actively seeking young digital talent  

StudentCareer+ targets a clear and growing market gap.

## 🛠 Tech & Tools

- **Design:** Figma  
- **Ideation & Research:** Claude, Gemini  
- **Documentation:** Samsung Notes  
- **Hardware:** Laptop  

Interactive prototype, screens, and video walkthrough available.

## 📌 Project Status

- High-fidelity interactive prototype
- Core user flows and monetization designed
- Ready for MVP development or investor pitch

## 👤 Author

**Founder & Product Designer (Solo Project)**  
- Product vision & strategy  
- UX/UI design  
- Market & competitor analysis  
- Business model & monetization  
- Branding & positioning  

## 📎 Links

- 🎨 Figma Prototype: *([https://www.figma.com/make/2I3XsdhkqT5E6XuPJJOeyf/StudentCareer-?t=nEjXONgirA4sMM9q-1](https://www.figma.com/make/2I3XsdhkqT5E6XuPJJOeyf/StudentCareer-?fullscreen=1&t=70PCJUCabsRNOIn9-1))*  
- 🎥 Product Walkthrough Video: *(https://youtu.be/DBPLVQDLaDI?si=FtfaBCgKoAn6tuA5)*    



## 🧠 Why This Project Matters

StudentCareer+ demonstrates:
- Strong product thinking  
- User-centered design  
- Business and monetization strategy  
- Ability to identify and solve real market problems  

This project represents a full-cycle product case from idea to scalable platform concept.

Instagram: https://www.instagram.com/maksym_sapiha?igsh=d253M2N0dWRubWN0

LinkedIn: https://www.linkedin.com/in/maksym-sapiha

